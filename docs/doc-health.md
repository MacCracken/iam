...
> **Last refresh**: 2026-08-23 (v1.1.7 cut — **P(-1) hardening sweep**: five findings filed and fixed, one of them (F-003) a live violation of ADR 0002's six-required-lines contract; new audit doc [`audit/2026-08-23-v1.1.7-audit.md`](audit/2026-08-23-v1.1.7-audit.md) supersedes the v1.0.0 doc; tests 122 → 141; `src/main.cyr` + `tests/iam.tcyr` normalized to canonical cyrfmt. Rows touched: `VERSION`, `CHANGELOG.md`, `docs/benchmarks.md`, `development/state.md`, and the whole Tier 5 audit table — which was itself two audits behind, having never gained a row for the v1.0.0 doc.) **Prior refresh**: 2026-08-23 (v1.1.6 cut — GPU-memory suffix (ADR 0003) + toolchain/dep-floor refresh: cyrius `6.2.37` → `6.5.35`, mihi `1.2.1` → `1.2.4`, ai-hwaccel `2.2.6` → `2.3.18`, `sakshi` added to the stdlib list, `lib/` re-vendored and ten orphans pruned). Rows touched this cut: `VERSION`, `cyrius.cyml`, `cyrius.lock`, `CHANGELOG.md`, `docs/benchmarks.md`, `development/state.md`, `development/roadmap.md`. **Note**: rows *not* listed there carry dates from the v0.9.0-era sweep and have not been re-read since — the row dates are honest, the blanket "every row reads ✅ Fresh" claim below is the stale part, and a full re-sweep is the next doc-health job. **Prior refresh**: 2026-05-19 v0.9.0 RC — F-001 sanitizer landed, follow-up audit filed. **Prior**: 2026-05-19 v0.8.0 — ADR 0002 accepted, output-shape reorder. | **Refresh cadence**: when docs are touched, update the affected row.
...
| `CHANGELOG.md` | 2026-08-23 | ✅ Fresh | **Source of truth per CLAUDE.md.** Through v1.1.7 (P(-1) hardening sweep). Refreshed every release. |
...
| `VERSION` | 2026-08-23 | ✅ Fresh | Single source of truth for version (`1.1.7` at last edit). `cyrius.cyml [package].version = "${file:VERSION}"` auto-tracks. |
...
| `2026-05-19-v0.9.0-audit.md` | 2026-05-19 | 🔵 Dated artifact — v0.9.0 RC follow-up audit. Verdict pass; F-001 RESOLVED; F-002 carries INFO. Superseded by the v1.0.0 doc; kept in-place. |
| `2026-05-20-v1.0.0-audit.md` | 2026-05-20 | 🔵 Dated artifact — mandatory mihi-major-bump audit (0.7.0 → 1.0.0). Verdict pass, no new findings, zero `src/` changes. Superseded by the v1.1.7 doc; kept in-place. **Ledger note**: this row did not exist until 2026-08-23 — the audit shipped without a doc-health entry, which is the same drift class the v1.1.6 header note flags. |
| `2026-08-23-v1.1.7-audit.md` | 2026-08-23 | ✅ Fresh — **current audit.** Full P(-1) sweep; first line-by-line `src/` re-walk since v0.9.0. **Verdict: not a pass on arrival** — five findings, one (F-003, HIGH) a live violation of ADR 0002's six-required-lines contract, all fixed and regression-tested in the same cut. Section A findings + reachability, Section B verification incl. every test confirmed red against the v1.1.6 source, Section C carry-forward (F-002 INFO) + syscall surface, Section D documentation audit. |

Per CLAUDE.md *Process P(-1)*: next renewal trigger is any mihi major bump or any mihi cut that changes probe return shape or scratch sizing (F-003's `IAM_VALUE_MAX` is sized against today's buffers), any change to `IAM_OUT_CAP` / `IAM_VALUE_MAX` / the renderer set, or a v2.0 reconsideration of colour (ADR 0001 §5) — whichever comes first.
